...
public partial class SpcSource(IOlieWebService ows) : ISpcSource
{
    private const string StormEventsUrl = "https://www.spc.noaa.gov/climo/reports/";

    public async Task<(string body, string etag)> DownloadNew(DateTime effectiveDate, CancellationToken ct)
    {
        var url = GetUrl(effectiveDate);
        var (status, etagValue, body) = await ows.ApiGet(url, null, ct);
    ...
    public async Task<(string body, string etag, bool isUpdated)> DownloadUpdate(
        DateTime effectiveDate, string etag, CancellationToken ct)
    {
        var url = GetUrl(effectiveDate);
        var etagHeader = new EntityTagHeaderValue(etag);
        var (status, etagValue, body) = await ows.ApiGet(url, etagHeader, ct);
