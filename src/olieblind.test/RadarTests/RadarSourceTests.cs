...
public class RadarSourceTests
{
    ...
    public async Task AddRadarInventoryAsync_AllSteps_ValidParameters()
    {
        ...
        ows.Setup(s => s.AwsList(RadarSource.LevelIiBucket, It.IsAny<string>(), null!, ct))
            .ReturnsAsync(items);

        // Act
        await testable.AddRadarInventory(cache, radar, effectiveTime, null!, ct);
    ...
    public void FindClosestRadar_Closest_ValidParameters()
    {
        ...
        // Act
        var result = testable.FindClosestRadars(sites, 45, -93);

        // Assert
        Assert.That(result[0].Id, Is.EqualTo("b"));
    ...
    public async Task GetRadarInventory_ShortCircuit_CacheHit()
    {
        ...
        // Act
        var result = await testable.GetRadarInventory(cache, radar, effectiveTime, ct);
    ...
    public async Task GetRadarInventory_Null_NoRecord()
    {
        ...
        var cache = new List<RadarInventoryEntity>() { new() };
    ...
    public async Task GetRadarInventory_UpdatesCache_RecordFound()
    {
        ...
        cosmos.Setup(s => s.RadarInventoryGet(radarId, effectiveDate, RadarSource.LevelIiBucket, ct))
