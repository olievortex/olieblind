...
public class SpcSourceTests
{
    ...
    [Test]
    public void DownloadNew_ThrowsException_Redirect()
    {
        // Arrange
        var ct = CancellationToken.None;
        var effectiveDate = new DateTime(2021, 7, 10);
        var ows = new Mock<IOlieWebService>();
        ows.Setup(s => s.ApiGet(It.IsAny<string>(), It.IsAny<EntityTagHeaderValue>(), ct))
            .ReturnsAsync((HttpStatusCode.Redirect, null, string.Empty));
        var testable = new SpcSource(ows.Object);

        // Act, Assert
        Assert.ThrowsAsync<Exception>(() => testable.DownloadNew(effectiveDate, ct));
    }
    ...
    public async Task DownloadNewAsync_Downloads_New()
    {
        ...
        // Act
        var (a, b) = await testable.DownloadNew(effectiveDate, ct);
    ...
    public async Task DownloadUpdateAsync_ShortCircuits_NotModified()
    {
        ...
        // Act
        var (_, __, result) = await testable.DownloadUpdate(effectiveDate, etag, ct);
    ...
    public void DownloadUpdateAsync_ThrowsException_NoETag()
    {
        ...
        // Act, Assert
        Assert.ThrowsAsync<ApplicationException>(() => testable.DownloadUpdate(effectiveDate, etag, ct));
    }
    ...
    public async Task DownloadUpdateAsync_Downloads_Update()
    {
        ...
        // Act
        var (a, b, c) = await testable.DownloadUpdate(effectiveDate, etag, ct);
