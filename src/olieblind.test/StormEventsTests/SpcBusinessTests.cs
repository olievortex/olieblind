...
public class SpcBusinessTests
{
    ...
    public async Task DailyDetailAsync_ShortCircuits_AlreadyComplete()
    {
        ...
        // Act
        await testable.AddDailyDetail(models, inventory, ct);

        // Assert
        cosmos.Verify(v => v.StormEventsReportUpdate(inventory, ct), Times.Never);
    }
    ...
    public async Task DailySummaryAsync_ShortCircuit_AlreadyComplete()
    {
        ...
        // Act
        await testable.AddDailySummary(inventory, null, string.Empty, ct);
    ...
    public async Task DailySummaryAsync_CompletesAllSteps_New()
    {
        ...
        var ct = CancellationToken.None;
        var cosmos = new Mock<IMyRepository>();
        cosmos.Setup(s => s.StormEventsDailySummaryGet("2010-07-18", 2010, ct))
        ...
        // Act
        await testable.AddDailySummary(inventory, model, string.Empty, ct);
    ...
    public async Task DownloadNewAsync_CompletesAllSteps_ValidParameters()
    {
        ...
        var cosmos = new Mock<IMyRepository>();
        var source = new Mock<ISpcSource>();
        source.Setup(s => s.DownloadNew(effectiveDate, ct)).ReturnsAsync(("a", "b"));
        var testable = new SpcBusiness(cosmos.Object, source.Object);

        // Act
        var result = await testable.DownloadNew(effectiveDate, ct);
    ...
    public async Task DownloadUpdateAsync_ShortCircuits_TooRecent()
    {
        ...
        var source = new Mock<ISpcSource>();
        var testable = new SpcBusiness(cosmos.Object, source.Object);
        var inventory = new StormEventsReportEntity { Timestamp = timestamp };

        // Act
        var result = await testable.DownloadUpdate(inventory, ct);
    ...
    public async Task DownloadUpdateAsync_ShortCircuits_NotModified()
    {
        ...
        var cosmos = new Mock<IMyRepository>();
        var source = new Mock<ISpcSource>();
        source.Setup(s => s.DownloadUpdate(It.IsAny<DateTime>(), It.IsAny<string>(), ct))
    ...
    public void GetFirstDayNumberForYear_StartOfYear_NotEventsDatabaseEra()
    {
        // Arrange
        const int year = 2026;
        var startOfYear = new DateTime(2026, 1, 1);
    ...
    public void GetLatest_ReturnsTopRow_MultipleRecords()
    {
        ...
        var effectiveDate = new DateTime(2021, 7, 10);
        var cosmos = new Mock<IMyRepository>();
        var inventory = new List<StormEventsReportEntity>
        {
